...
export default function Navigation() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['home', 'about', 'internships', 'projects', 'skills', 'contact'];
                ...
                <div className="flex flex-col gap-6 mt-8">
                  {navItems.map((item) => (
                    <button
                      key={item.id}
                      onClick={() => scrollToSection(item.id)}
                      className="text-left text-lg font-medium text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200"
                    >
                      {item.label}
                    </button>
                  ))}
